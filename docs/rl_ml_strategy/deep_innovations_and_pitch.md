...
> **Overview:** This document compiles the core technical, mathematical, architectural, and operational innovations engineered into AeroOptima for AEROTHON 2026.
...
- **Outer Loop (Hardware Sizing):** DEAP Genetic Algorithm searches continuous design space for Engine Power ($P_{eng} \in [30, 120]\text{ kW}$) and Battery Capacity ($E_{batt} \in [5, 50]\text{ kWh}$).
...
- **Single-Engine Redundancy (STANAG 4671):** If the battery fails, engine alone provides **60 kW** vs. 55.8 kW required cruise $\to$ indefinite flight.
...
$$\text{SFC}(L_f) = 
\begin{cases} 
\text{SFC}_{\text{base}} & L_f \ge 0.8 \\
\text{SFC}_{\text{base}} \cdot \left[1.0 + 0.15 \cdot \frac{0.8 - L_f}{0.3}\right] & 0.5 \le L_f < 0.8 \\
\text{SFC}_{\text{base}} \cdot \left[1.15 + 0.25 \cdot \frac{0.5 - L_f}{0.5}\right] & L_f < 0.5 
\end{cases}$$
